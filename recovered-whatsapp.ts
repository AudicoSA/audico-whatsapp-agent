...
export class WhatsAppClient {
  ...
  async sendProductCards({ to, products, header }: ProductCardMessage): Promise<boolean> {
    ...
    products.forEach((product, index) => {
      ...
      text += `   ≡ƒÆ░ R${product.price.toLocaleString()}\n`;
      text += `   ≡ƒôª SKU: ${product.sku}\n`;
      if (product.image) {
        text += `   ≡ƒû╝∩╕Å ${product.image}\n`;
      }
      text += '\n';
    });
  ...
  async sendQuoteSummary(
    ...
    let text = '≡ƒ¢Æ *Your Quote Summary*\n\n';
    let total = 0;
    ...
    text += `ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n`;
    text += `*Total: R${total.toLocaleString()}*\n`;
    text += `_(excl. VAT & delivery)_\n\n`;

    if (quoteId) {
      text += `≡ƒôï Quote Ref: ${quoteId}\n\n`;
    }
